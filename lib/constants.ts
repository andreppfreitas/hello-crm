...
> = {
  ...
  // Payments
  coe_deposit_paid:          { label: "CoE Deposit Paid",                      phase: "payments",   color: "text-orange-300",   bg: "bg-orange-500/20 border-orange-500/30",  dot: "bg-orange-400",   order: 16 },
  coe_issued:                { label: "CoE Issued",                            phase: "payments",   color: "text-yellow-300",   bg: "bg-yellow-500/20 border-yellow-500/30",  dot: "bg-yellow-400",   order: 17 },
  coe_confirmed:             { label: "CoE Confirmed by Student",              phase: "payments",   color: "text-lime-300",     bg: "bg-lime-500/20 border-lime-500/30",      dot: "bg-lime-400",     order: 18 },
  oshc_payment:              { label: "OSHC Payment Collected",                phase: "payments",   color: "text-teal-300",     bg: "bg-teal-500/20 border-teal-500/30",      dot: "bg-teal-400",     order: 19 },
  oshc_issued:               { label: "OSHC Issued",                           phase: "payments",   color: "text-cyan-300",     bg: "bg-cyan-500/20 border-cyan-500/30",      dot: "bg-cyan-400",     order: 20 },
  visa_fee_paid:             { label: "Visa Fee Paid",                         phase: "payments",   color: "text-green-300",    bg: "bg-green-500/20 border-green-500/30",    dot: "bg-green-400",    order: 21 },
  // Documents
  documents_signed:          { label: "Student Signed Documents",              phase: "enrollment", color: "text-purple-300",   bg: "bg-purple-500/20 border-purple-500/30",  dot: "bg-purple-400",   order: 15 },
...
> = {
  ...
  enrollment: { label: "Enrollment",  color: "text-purple-300",   headerBg: "bg-purple-500/10 border-purple-500/20",  stages: ["application_sent", "school_requested_docs", "offer_letter_received", "final_quote_sent", "contract_sent", "read_carefully_email", "documents_signed"] },
  documents:  { label: "Documents",   color: "text-teal-300",     headerBg: "bg-teal-500/10 border-teal-500/20",      stages: ["visa_checklist_sent", "student_uploading_docs", "gs_letter_draft_sent", "gs_letter_approved", "documents_complete"] },
...
export const TASK_TEMPLATES: Record<PipelineStage, string[]> = {
  // Leads
  new_lead:                  ["Conferir os dados do cadastro", "Ligar para o aluno", "Registrar o que ele procura"],
  first_contact:             ["Mandar WhatsApp de boas-vindas", "Apresentar a Hello Australia", "Entender objetivo e prazo do aluno"],
  // Qualifying
  initial_docs_requested:    ["Enviar a lista de documentos", "Cobrar se não responder em 48h", "Tentar um contato alternativo"],
  initial_docs_received:     ["Conferir os documentos recebidos", "Agendar a reunião de consultoria", "Confirmar a reunião por WhatsApp"],
  meeting_scheduled:         ["Confirmar data e horário", "Preparar as opções para apresentar", "Enviar o convite da reunião"],
  quotation_prepared:        ["Montar as opções de curso", "Calcular o custo total", "Enviar a cotação"],
  // Proposal
  followup:                  ["Checar se o aluno viu a cotação", "Responder dúvidas e objeções", "Definir a data do próximo contato"],
  student_approved_quotation:["Coletar os dados para a aplicação", "Explicar os próximos passos", "Pedir os documentos da escola"],
  // Enrollment
  application_sent:          ["Preencher o formulário da escola", "Enviar a aplicação", "Confirmar o recebimento com a escola"],
  school_requested_docs:     ["Avisar o aluno do que falta", "Enviar a lista à escola", "Cobrar o envio dos documentos"],
  offer_letter_received:     ["Conferir os termos da offer letter", "Validar curso, duração, campus, CRICOS e intake", "Explicar a offer ao aluno"],
  final_quote_sent:          ["Montar a cotação final com todos os custos", "Enviar ao aluno", "Cobrar a confirmação"],
  contract_sent:             ["Enviar o contrato da Hello", "Explicar as cláusulas ao aluno", "Cobrar a assinatura"],
  read_carefully_email:      ["Escrever o e-mail 'Leia com Atenção'", "Incluir todas as datas e pagamentos", "Copiar o time de visto"],
  documents_signed:          ["Receber os documentos assinados", "Arquivar na pasta do aluno", "Enviar as instruções do depósito do CoE"],
  // Payments
  coe_deposit_paid:          ["Confirmar o depósito com a escola", "Pedir a emissão do CoE", "Atualizar a ficha do aluno"],
  coe_issued:                ["Receber o CoE da escola", "Enviar o CoE ao aluno", "Confirmar que o aluno recebeu"],
  coe_confirmed:             ["Explicar o que é o OSHC", "Enviar as instruções de pagamento", "Cobrar o pagamento"],
  oshc_payment:              ["Confirmar o pagamento do OSHC", "Emitir o recibo", "Solicitar a apólice à seguradora"],
  oshc_issued:               ["Receber a apólice", "Enviar a apólice ao aluno", "Enviar as instruções da taxa de visto"],
  visa_fee_paid:             ["Confirmar o pagamento da taxa", "Conferir se o CoE e o OSHC estão em mãos", "Preparar o checklist de visto"],
  // Documents
  visa_checklist_sent:       ["Enviar o checklist ao aluno", "Explicar cada documento exigido", "Definir o prazo de envio"],
  student_uploading_docs:    ["Acompanhar os envios", "Conferir cada documento recebido", "Pedir correção do que estiver errado"],
  gs_letter_draft_sent:      ["Escrever o rascunho da GS Letter", "Enviar ao aluno para revisão", "Aguardar a aprovação dele"],
  gs_letter_approved:        ["Finalizar a GS Letter", "Juntar ao pacote de documentos", "Confirmar que não falta nada"],
  documents_complete:        ["Revisar o pacote inteiro", "Conferir a validade do passaporte", "Preparar a aplicação para protocolo"],
  // Visa
  visa_lodged:               ["Protocolar a aplicação no ImmiAccount", "Enviar a confirmação ao aluno", "Informar o prazo estimado"],
  medical_requested:         ["Avisar o aluno sobre o exame médico", "Enviar a lista de clínicas credenciadas", "Cobrar a realização do exame"],
  visa_granted:              ["Avisar o aluno da aprovação", "Enviar o checklist pré-embarque", "Confirmar acomodação e chegada"],
};
